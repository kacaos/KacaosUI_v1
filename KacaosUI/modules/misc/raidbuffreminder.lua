local T, C, L = unpack(Tukui) -- thanks goes to epicgrim
if C.misc.raidbuffreminder == false then return end

-- Settings
local specialbuff1 = C.misc.specialbuff
...
local function OnAuraChange(self, event, arg1, unit)
	...
	local spellname = select(1, GetSpellInfo(specialbuff1))
	if UnitAura("player", spellname) then
		SpecialBuffFrame1:SetAlpha(0.2)
		SpecialBuffFrame1.t:SetTexture(select(3, GetSpellInfo(specialbuff1)))
	else
		SpecialBuffFrame1:SetAlpha(1)
		SpecialBuffFrame1.t:SetTexture(select(3, GetSpellInfo(specialbuff1)))
	end
end

local fsize = 171
local bsize = ((fsize - 9) / 7)
...
do
	...
	CreateButton("SpecialBuffFrame1", Spell6Frame, false)
end
