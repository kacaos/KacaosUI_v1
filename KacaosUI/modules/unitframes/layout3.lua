...
for _, frame in pairs(units) do
	...
	button.Glow:Kill()
		...
		if T.level ~= MAX_PLAYER_LEVEL then
			...
			Experience:Point("BOTTOM", UIParent, 0, 8)
		...
		if T.level == MAX_PLAYER_LEVEL then
			...
			Reputation:Point("BOTTOM", UIParent, 0, 8)
	...
	if unit == "target" then
		...
		TargetCast.button:SetTemplate("Transparent")
		...
		TukuiTarget.Debuffs:Point("CENTER", self.Power, "CENTER", 1, 55)
...
-------------------------------------------------------------------
	...
	TukuiBoss1:Point("CENTER", UIParent, 0, 0)
